...
import AddToSkillModal from "./AddToSkillModal";
import {
  createPage,
  listAllPages,
  listAllTables,
...
describe("AddToSkillModal", () => {
  beforeEach(() => {
    ...
    vi.mocked(updateSkill).mockResolvedValue({} as Awaited<ReturnType<typeof updateSkill>>);
  });
  ...
  it("offers one add surface for existing, URL, file, and note flows", async () => {
    ...
    expect(screen.getByRole("heading", { name: "Add to Skill" })).toBeInTheDocument();
  ...
  it("adds a pasted URL as a page in the target Skill", async () => {
    const onAdded = vi.fn();
    const onClose = vi.fn();
