...
One planner serving several travel agencies. Each agency is an **org**: what
...
three weeks. It warns them off, citing "a peer org" — and nothing in
...
- `read(org, script)` → `POST /api/v1/me/vfs` — run a script against that
  customer's view. `/memory` is the shared wiki, `/files` their notepad and
  files, `/sessions` their transcripts.
- `record(org, orgName, session, events)` → `POST /api/v1/me/sessions/events/batch`
  — one event per turn, tagged with the customer's org id. First sight of an
  id creates the org.

`lib/agent.ts` is the whole loop: read, answer, record. Everything else —
`api/orgs`, `api/transcript`, the page — is presentation, and a real product
would not need it.
