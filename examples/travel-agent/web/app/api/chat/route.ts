...
export async function POST(request: Request) {
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 400,
      system: SYSTEM,
      messages: [{ role: "user", content: `What we know:\n${ctx || "(nothing yet)"}\n\n${question}` }],
    }),
  });
  if (!res.ok) return Response.json({ error: await res.text() }, { status: 502 });
  const body = await res.json();
  const reply = body.content.filter((b: { type: string }) => b.type === "text").map((b: { text: string }) => b.text).join("");

  await record(org, orgName, session, [["user_message", question], ["assistant_message", reply]]);
