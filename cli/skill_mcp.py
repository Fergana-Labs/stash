...
def _json_servers(path: Path, key: str, servers: dict[str, str], owned: set[str], entry) -> None:
    data = _read_json(path)
    table = data.get(key, {})
    for name in owned - set(servers):
        table.pop(name, None)
    for name, url in servers.items():
        table[name] = entry(url)
    data[key] = table
    _write_json(path, data)


def _write_claude(servers, owned, bearer) -> None:
    _json_servers(
        Path.home() / ".claude.json",
        "mcpServers",
        servers,
        owned,
        lambda url: {"type": "http", "url": url, "headers": {"Authorization": bearer}},
    )


def _write_cursor(servers, owned, bearer) -> None:
    _json_servers(
        Path.home() / ".cursor" / "mcp.json",
        "mcpServers",
        servers,
        owned,
        lambda url: {"url": url, "headers": {"Authorization": bearer}},
    )


def _write_gemini(servers, owned, bearer) -> None:
    _json_servers(
        Path.home() / ".gemini" / "settings.json",
        "mcpServers",
        servers,
        owned,
        lambda url: {"httpUrl": url, "headers": {"Authorization": bearer}},
    )


def _write_opencode(servers, owned, bearer) -> None:
    _json_servers(
...
def _write_codex(servers, owned, bearer) -> None:
    ...
    for name, url in servers.items():
        lines += [
            ...
            f"http_headers = {{ Authorization = {json.dumps(bearer)} }}",
            "",
        ]
...
def sync(root: Path, agents: list[str]) -> list[str]:
    ...
    if servers and not api_key:
        raise RuntimeError("not signed in: run `stash signin` before syncing skill tool servers")
    bearer = f"Bearer {api_key}"

    written = [agent for agent in agents if agent in _WRITERS]
    for agent in written:
        _WRITERS[agent](servers, owned, bearer)
