...
async def list_workspace_sessions(workspace_id: UUID) -> list[dict]:
    ...
    rows = await pool.fetch(
        "SELECT s.session_id, "
        "       COALESCE(NULLIF(s.agent_name, ''), events.agent_name, '') AS agent_name, "
        "       COALESCE(events.event_count, 0)::INT AS event_count, "
        "       COALESCE(events.size_bytes, 0)::BIGINT AS size_bytes, "
        "       COALESCE(events.started_at, s.started_at) AS started_at, "
        "       COALESCE(events.last_at, s.finished_at, s.started_at) AS last_at, "
        "       s.id AS bundle_id, "
        "       share.slug AS bundle_slug, "
        "       s.status AS status, "
        ...
        "    WHERE he.workspace_id = s.workspace_id AND he.session_id = s.session_id"
        ") events ON TRUE "
        "LEFT JOIN LATERAL ("
        "    SELECT slug FROM share_links "
        "    WHERE target_type = 'session' AND target_id = s.id AND revoked_at IS NULL "
        "    ORDER BY created_at LIMIT 1"
        ") share ON TRUE "
        "LEFT JOIN LATERAL ("
        "    SELECT COUNT(*)::INT AS artifact_count "
        "    FROM session_artifacts sa WHERE sa.session_id = s.id"
        ") artifacts ON TRUE "
