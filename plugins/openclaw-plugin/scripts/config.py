"""Openclaw plugin config: reads from ~/.octopus/config.json (CLI config).

Openclaw has no plugin-level userConfig surface, so we piggyback on the CLI
...
DATA_DIR = Path(os.environ.get(
    "OCTOPUS_OPENCLAW_DATA",
    Path.home() / ".octopus/plugins/openclaw",
))
