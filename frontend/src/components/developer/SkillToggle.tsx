...
/** Whether this user's sessions feed the shared anonymized skill. A real switch
...
export default function SkillToggle({ user, onChanged }: { user: EndUser; onChanged: () => void }) {
  ...
  async function toggle() {
    ...
    setError(null);
    try {
      await updateUser(user.id, { share_skill: !user.share_skill });
      onChanged();
    } catch (e) {
      setError(e instanceof Error ? e.message : "Could not change the skill setting");
    } finally {
      setSaving(false);
    }
  }
      ...
      >
        <span
          className={cn(
            ...
            user.share_skill ? "bg-brand-500" : "bg-border group-hover:bg-muted-foreground/40",
          )}
        >
          <span
            className={cn(
              "absolute top-0.5 h-4 w-4 rounded-full bg-white shadow-sm transition-[left]",
              user.share_skill ? "left-[18px]" : "left-0.5",
            )}
          />
        </span>
        <span
