...
// on top of a StashCard. Click is swallowed so the parent <Link> doesn't
// navigate to the stash detail page.
export default function ForkStashCardButton({ slug, sourceWorkspaceId }: Props) {
  ...
  async function openPicker() {
    if (!getToken()) {
      router.push(`/login?next=${encodeURIComponent(`/stashes/${slug}?action=add`)}`);
      return;
    }
