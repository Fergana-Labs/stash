...
Restored: this migration was lost from main (a concurrent push to the
shared checkout swept the file) while the code that depends on the
column stayed — leaving the chain broken and prod stamped at a revision
with no file. Re-adding it repairs the chain. ``IF NOT EXISTS`` keeps it
safe on databases that already ran the original (prod).
...
def upgrade() -> None:
    op.execute(
        "ALTER TABLE paste_comments ADD COLUMN IF NOT EXISTS edit_token TEXT NOT NULL DEFAULT ''"
    )
