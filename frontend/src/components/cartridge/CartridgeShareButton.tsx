...
export default function CartridgeShareButton({
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      const [shares, me] = await Promise.all([
        listObjectShares("stash", stash.id),
        getMe(),
      ]);
  ...
  async function addMember(e: FormEvent) {
    ...
    setMemberMessage("");
    try {
      await shareObjectByEmail("stash", stash.id, email, newMemberPermission);
      await loadMembers();
      setMemberEmail("");
  ...
  async function changeMemberPermission(share: ObjectShare, permission: SharePermission) {
    ...
    setMemberMessage("");
    try {
      await shareObjectByEmail("stash", stash.id, share.email, permission);
      await loadMembers();
  ...
  async function deleteMember(share: ObjectShare) {
    ...
    if (!confirm("Remove this person from the cartridge?")) return;

    setMemberBusy(true);
    setMemberMessage("");
    try {
      await unshareObject("stash", stash.id, "user", share.principal_id);
      await loadMembers();
      resetStashNavigationCache();
    } catch (e) {
