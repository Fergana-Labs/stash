...
async def create_or_update_invite(
    *,
    ...
    row = await pool.fetchrow(
        """
        INSERT INTO skill_invites
          (skill_id, recipient_user_id, invited_by_user_id, permission, status,
           target_workspace_id, responded_at)
        VALUES ($1, $2, $3, $4, 'pending', NULL, NULL)
        ON CONFLICT (skill_id, recipient_user_id) DO UPDATE
        SET invited_by_user_id = EXCLUDED.invited_by_user_id,
            ...
            responded_at = NULL,
            updated_at = now()
        RETURNING id, skill_id, recipient_user_id, invited_by_user_id, permission,
...
async def delete_pending_invite(skill_id: UUID, recipient_user_id: UUID) -> None:
    pool = get_pool()
    await pool.execute(
        "DELETE FROM skill_invites "
        "WHERE skill_id = $1 AND recipient_user_id = $2 AND status = 'pending'",
        skill_id,
        recipient_user_id,
    )
...
async def mark_invite_accepted_for_skill(
    *,
    skill_id: UUID,
    user_id: UUID,
    workspace_id: UUID,
