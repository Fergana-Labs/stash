...
async def list_workspace_sessions(workspace_id: UUID, user_id: UUID) -> list[dict]:
    ...
    rows = await pool.fetch(
        ...
        "       (ARRAY_AGG(h.content ORDER BY "
        "          CASE "
        "            WHEN h.event_type IN ('user_message', 'user_prompt', 'prompt', 'message', 'user') THEN 0 "
        "            WHEN h.event_type IN ('assistant_message', 'assistant') THEN 1 "
        "            ELSE 2 "
        "          END, h.created_at, h.id) "
        "        FILTER (WHERE NULLIF(BTRIM(h.content), '') IS NOT NULL))[1] AS title_source, "
