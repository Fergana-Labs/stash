...
import { createSkill, getWorkspaceSidebar } from "../../lib/api";
...
describe("SkillShareModal session sharing", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("resolves a route session id to the session row uuid before creating a Skill", async () => {
    render(
      <ShareModalProvider>
    ...
    await screen.findByRole("heading", { name: "Share session as Skill" });
    ...
    await waitFor(() =>
      expect(
        screen.queryByRole("heading", { name: "Share session as Skill" })
      ).not.toBeInTheDocument()
    );
  });

  it("can create a private Skill", async () => {
    render(
      <ShareModalProvider>
        <OpenSessionShareButton />
        <SkillShareModal />
