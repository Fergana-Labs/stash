...
interface PickerRow {
  ...
  sub: string;
  /** What we send when we add it to the skill. */
  spec: SkillItemSpec;
}

export default function AddToSkillModal({
  open,
  onClose,
  skillId,
  workspaceId,
  ...
  function nextItemsFor(newItems: SkillItemSpec[]): SkillItemSpec[] {
    return [
  ...
  async function addNewItems(newItems: SkillItemSpec[]) {
    await updateSkill(skillId, { items: nextItemsFor(newItems) });
    onAdded();
    onClose();
  }
  ...
  async function attachExternalSkill() {
    const slug = skillSlugFromInput(externalUrl);
    if (!slug) {
      setError("Paste a skill URL or slug.");
      return;
    }
    setSubmitting(true);
    setError(null);
    try {
      await forkSkill(workspaceId, slug);
      onAdded();
      onClose();
    } catch (e) {
      if (e instanceof ApiError && e.status === 404) {
        setError("Couldn't find that skill. Check the URL.");
      } else {
        setError(e instanceof Error ? e.message : "Couldn't attach external skill");
      }
    } finally {
      setSubmitting(false);
    }
    ...
    >
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 className="m-0 font-display text-[17px] font-semibold">Add to Skill</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex flex-wrap gap-1 border-b border-border bg-surface px-2 py-2">
          ...
          <ModeBtn label="External skill" active={mode === "external"} onClick={() => setMode("external")} />
        </div>
        ...
        {mode === "existing" ? (
          ...
          <div className="px-4 py-4">
            <p className="m-0 text-[12.5px] text-muted">
              Save a URL as a page in this workspace and add it to this skill.
            </p>
            <input
              type="url"
              value={urlValue}
          ...
          <div className="px-4 py-4">
            <p className="m-0 text-[12.5px] text-muted">
              Upload files to this workspace and add them to this skill.
            </p>
          ...
          <div className="px-4 py-4">
            <p className="m-0 text-[12.5px] text-muted">
              Create a page from a note and add it to this skill.
            </p>
            <textarea
              value={noteValue}
          ...
          <div className="px-4 py-4">
            <p className="m-0 text-[12.5px] text-muted">
              Paste a public skill URL or slug. We&apos;ll fork its items into a new skill inside
              this workspace and link it as an external skill.
            </p>
            <input
              type="text"
              value={externalUrl}
              onChange={(e) => setExternalUrl(e.target.value)}
              placeholder="https://app.joinstash.ai/skills/example-slug"
              className="mt-3 w-full rounded-md border border-border bg-surface px-2.5 py-1.5 text-[13px] text-foreground placeholder:text-muted focus:border-[var(--color-brand-400)] focus:outline-none"
            />
          </div>
        )}
