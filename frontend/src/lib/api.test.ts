...
describe("token management", () => {
  ...
  it("setToken stores token in localStorage", () => {
    ...
    expect(localStorage.getItem("boozle_token")).toBe("test-token-123");
  });

  it("clearToken removes token from localStorage", () => {
    localStorage.setItem("boozle_token", "test-token-123");
    clearToken();
    expect(localStorage.getItem("boozle_token")).toBeNull();
  });
});
...
describe("apiFetch", () => {
  ...
  it("includes Authorization header when token is set", async () => {
    ...
    localStorage.setItem("boozle_token", "my-token");
