...
export default async function CohortsAdminPage({
  ...
  const url = `${apiUrl}/api/v1/admin/cohorts/engagement?bucket=${bucket}&mode=${mode}`;
  ...
  return <CohortClient data={data} bucket={bucket} mode={mode} />;
}
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <div className="mx-auto max-w-[720px] px-7 py-20">
        <h1 className="font-display text-[28px] font-black tracking-[-0.03em] text-ink">
          {title}
        </h1>
        <pre className="mt-4 whitespace-pre-wrap rounded-md border border-border-subtle bg-background p-4 font-mono text-[12px] text-dim">
          {body}
        </pre>
      </div>
    </main>
