"""Memory store router: structured agent event storage."""
...
async def _check_store_access(workspace_id: UUID, store_id: UUID, user_id: UUID) -> None:
    has_access = await permission_service.check_access(
        "memory_store", store_id, user_id, workspace_id=workspace_id,
    )
    if not has_access:
        raise HTTPException(status_code=403, detail="No access to this memory store")
...
async def create_store(
    workspace_id: UUID, req: MemoryStoreCreateRequest,
...
async def push_event(
    workspace_id: UUID, store_id: UUID, req: MemoryEventCreateRequest,
    current_user: dict = Depends(get_current_user),
):
    await _check_store_access(workspace_id, store_id, current_user["id"])
...
async def push_events_batch(
    workspace_id: UUID, store_id: UUID, req: MemoryEventBatchRequest,
    current_user: dict = Depends(get_current_user),
):
    await _check_store_access(workspace_id, store_id, current_user["id"])
...
async def get_permissions(
    ...
    await _check_member(workspace_id, current_user["id"])
    perms = await permission_service.get_permissions("memory_store", store_id)
...
async def set_visibility(
    ...
    await permission_service.set_visibility("memory_store", store_id, req.visibility)
...
async def add_share(
    ...
    share = await permission_service.add_share(
        "memory_store", store_id, req.user_id, req.permission, current_user["id"],
    )
...
async def remove_share(
    ...
    await permission_service.remove_share("memory_store", store_id, user_id)
